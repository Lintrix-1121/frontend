...
const useBlogStore = create(
  persist(
    (set, get) => ({
      ...
      createBlog: async (formData) => {
        ...
        try {
          ...
          if (result.success) {
            ...
            // Add helper methods to the new blog
            const blogWithHelpers = addBlogHelpers(result.data);
            set((state) => ({
      ...
      updateBlog: async (blogId, formData) => {
        ...
        try {
          ...
          if (result.success) {
            ...
            // Add helper methods to the updated blog
            const blogWithHelpers = addBlogHelpers(result.data);
            set((state) => ({
      ...
      // Delete blog, incrementViews, likeBlog, shareBlog, getBlogStatistics, 
      // clearCurrentBlog, clearError, clearSearchQuery, resetFilters, 
      // getFilteredBlogs, getPublishedBlogs, getDraftBlogs, getArchivedBlogs,
      // getFeaturedBlogs, getBlogById, getBlogBySlug, getBlogsByAuthorId,
      // getRecentBlogs, getMostViewedBlogs, getMostLikedBlogs, clearStore
      
      // NOTE: All these getter methods will now return blogs with helper methods
...
//           if (result.success) {
//             console.log('✅ [BLOG STORE] Blog fetched:', result.data);
//             set({ currentBlog: result.data, loading: false });
//           } else {
//             console.error('❌ [BLOG STORE] Failed to fetch blog:', result.error);
//             set({ error: result.error, loading: false });
//           }
...
//             set((state) => ({
//               blogs: state.blogs.map(blog => 
//                 blog.blogId === blogId ? result.data : blog
//               ),
//               currentBlog: state.currentBlog?.blogId === blogId ? result.data : state.currentBlog,
//               loading: false,
//               error: null
//             }));
...
//         try {
//           console.log('🗑️ [BLOG STORE] Deleting blog ID:', blogId);
...
//         try {
//           console.log('👁️ [BLOG STORE] Incrementing views for blog:', blogId);
...
//         try {
//           console.log('❤️ [BLOG STORE] Liking blog:', blogId);
